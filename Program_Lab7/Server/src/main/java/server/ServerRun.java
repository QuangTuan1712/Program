    ...
    public void start() {
        try {
            ...
            while (true) {
                ...
                System.out.println("Client '" + msg.getUser().getName() + "' at " + remoteAdd + "  sent: " + msg.getString());
                logger.info("Received  [ " + msg.getString() + " ]  from " + remoteAdd + ".\n");

                String[] news = msg.getString().split(";");
                String message;
                if (news[0].equals("save")) {
                    message = "Command '" + news[0] + "' not found. Use help to display commands in program\n";
                } else {
//                    message = commandReader.executeCommand(msg);
                    message = forkJoinPool.invoke(new RequestTask(commandReader, msg));
                }
                fixedPool.execute(() -> {
                    try {
                        ...
                        System.out.println("\n Error while sending answer to the client");
                        logger.warning("Error while sending answer to the client");
                    }
                });
            }
        } catch (IOException e) {
