...
public class ServerCommandReader{
    ...
    public ServerCommandReader(ServerReader reader) {
        ...
        this.loginCommand = new LoginCommand(reader);
        this.registerCommand = new RegisterCommand(reader);
    }
    ...
    public String executeCommand(NoName s) {
        ...
        try{
            switch (msg[0]) {
                ...
                case "insert": answer = insertCommand.execute(s.getUser().getName() + ":" + msg[1], s.getLabWork()); break;
                case "update": answer = updateCommand.execute(s.getUser().getName() + ":" + msg[1], s.getLabWork()); break;
                case "remove_key": answer = removeKeyCommand.execute(s.getUser().getName() + ":" + msg[1]); break;
                case "clear": answer = clearCommand.execute(s.getUser().getName()); break;
                ...
                case "remove_lower": answer = removeLowerCommand.execute(s.getUser().getName(), s.getLabWork()); break;
                case "history": answer = displayHistory(); break;
                case "replace_if_greater": answer = replaceIfGreaterCommand.execute(s.getUser().getName() + ":" + msg[1], s.getLabWork()); break;
                ...
                case "#login*": answer = loginCommand.execute(s.getUser().toString()); break;
                case "#register*": answer = registerCommand.execute(s.getUser().toString()); break;
