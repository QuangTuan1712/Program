...
public class LabWork implements Comparable<LabWork> {
    ...
    private final java.time.LocalDateTime creationDate = java.time.LocalDateTime.now(); //Поле не может быть null, Значение этого поля должно генерироваться автоматически
 ...
 */
    ...
    public void display() {
        System.out.format("  Id:  %08d%n", getId());
        ...
        if (getTunedInWorks() != null) {
            System.out.print(getTunedInWorks());
        }
        System.out.print("\n  Average Point: ");
        if (getAveragePoint() != null) {
            System.out.print(getAveragePoint());
        }
        System.out.print("\n  Difficulty: ");
        if (getDifficulty() != null) {
            System.out.print(getDifficulty());
        }
        System.out.print("\n  Author:");
        if (getAuthor() != null) {
            System.out.print("      { Name: " + author.getName() + ",  Weight: " + author.getWeight() + ",  Location:");
            if (getAuthor().getLocation() != null) { System.out.print("      { X: " + author.getLocation().getX() + ",  Y: " + author.getLocation().getY() +
                    ",  Z: " + author.getLocation().getZ() + ",  Name: "); }
            if (getAuthor().getLocation().getName() != null) {
                System.out.print(author.getLocation().getName() + " } }.");
